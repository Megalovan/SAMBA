...
def ULA_DFT_codebook(nseg, n_antenna, spacing = 0.5):
    ...
    for i, theta in enumerate(azimuths):
        arr_response_vec = [-1j*2*np.pi*k*spacing*np.sin(theta) for k in range(n_antenna)]      # arr_response_vec：64
...
def ULA_Gauss_Win(n_antenna, alpha = 5, spacing = 0.5):
    n = np.arange(1, n_antenna+1)
    delta = (n_antenna-1)/(2*alpha)
    win_gauss = np.exp(-(1/2)*np.multiply((n/delta), (n/delta)))
    win_gauss = win_gauss/np.linalg.norm(win_gauss, ord=2)
    return win_gauss

def ULA_Gauss_codebook(nseg, n_antenna, spacing=0.5):
    codebook_all = np.zeros((nseg, n_antenna), dtype=np.complex_)
    thetas = DFT_angles(nseg)
    azimuths = np.arcsin(1/spacing*thetas)
    win_gauss = ULA_Gauss_Win(n_antenna)
    for i,theta in enumerate(azimuths):
        arr_response_vec = [-1j*2*np.pi*k*spacing*np.sin(theta) for k in range(n_antenna)]
        codebook_all[i, :] = np.multiply(win_gauss, np.exp(arr_response_vec))
    return codebook_all
