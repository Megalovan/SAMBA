...
#%% complex dense layer numeral calculations
class PhaseShifter(Module):
    """
    This module is a pytorch implementation of the 
    complex Dense layer for phase shifters satisfying 
    constant modulus constraint. It is insipred by the 
    implemention by the orignal authors of the Deep Complex Networks paper
    ...
    scale: float
    theta: Tensor
    # PhaseShifter(in_features = 2*n_antenna, out_features = n_wide_beam, 
    # scale = np.sqrt(n_antenna), theta = theta)
    ...
    def reset_parameters(self, theta = None) -> None:
        if theta is None:
            init.uniform_(self.theta, a = 0, b = 2*np.pi)            # initialize by uniform distribution phase
        else:
            assert theta.shape == (self.in_dim, self.out_features)
            self.theta = Parameter(theta) 
        self.real_kernel = (1 / self.scale) * torch.cos(self.theta)  # scale is for normalization
        self.imag_kernel = (1 / self.scale) * torch.sin(self.theta)  #
    
    def forward(self, inputs: Tensor) -> Tensor:                     # input is supposed to be separeted in real and image part
        # theta (n_antenna, n_wide_beam), 64x6 for instance
        ...
        # [ W_R, -W_I; W_I, W_R]  (mentioned in 3.2 (2))
...
#%% CNN_Classifier -used in Trainalble + MPL
class Beam_Classifier(nn.Module):
    def __init__(self, n_antenna, n_wide_beam, n_narrow_beam, trainable_codebook = True, theta = None, complex_codebook = None, noise_power = 0.0, norm_factor = 1.0):
        ...
        self.compute_power = ComputePower(2*n_wide_beam)        # compute power with real and image part seperately
        self.relu = nn.ReLU()
    ...
    def forward(self, x):
        # The architecture of the proposed NN, including the probing 
        # codebook W and the beam selection function f.
        if self.trainable_codebook:
            # x is var_X_batch in the shape of (batch_size, n_narrow_beam)
            # beamforming signal in the shape of (batch_size, 2*n_wide_beam)
            bf_signal = self.codebook(x)
        else:
            self.codebook.to(x.device)
        ...
        noise_vec = noise_vec.to(x.device)
        bf_signal = bf_signal + noise_vec   # first and second half both count for energy, so + sqrt(noise_power/2)
        # bf_power is in the shape of (batch_size, wide_beam_number)
        bf_power = self.compute_power(bf_signal)
        # dim: n_wide_beam -> 2*n_wide_beam -> 3*n_wide_beam -> n_narrow_beam
    ...
    def get_codebook(self) -> np.ndarray:
        if self.trainable_codebook:
            return self.codebook.get_weights().cpu().detach().clone().numpy()
        else:
            return self.complex_codebook

#%% compute dimensions of convolution layer
...
#%% RSRP_CNN_Classifier, called in Beam_Classifier_CNN_RSRP
...
class RSRP_CNN_Classifier(nn.Module):
    def __init__(self, n_wide_beam, n_narrow_beam):
        ...
        output_dim = conv_dimension(input_dim = output_dim, kernel_size = 2, stride = 1)
        self.flatten_dim = 64*int(output_dim)       # why?
        self.dense1 = nn.Linear(in_features = self.flatten_dim, out_features = n_narrow_beam)
...
#%% Beam_Classifier_CNN_RSRP -used in Trainable + CNN
class Beam_Classifier_CNN_RSRP(nn.Module):
    def __init__(self, n_antenna, n_wide_beam, n_narrow_beam, trainable_codebook = True, theta = None, complex_codebook = None, noise_power = 0.0, norm_factor = 1.0):
        ...
        self.norm_factor = float(norm_factor)
        if trainable_codebook:
            # PhaseShifter(input features, output features, scale, theta)
            self.codebook = PhaseShifter(in_features = 2*n_antenna, out_features = n_wide_beam, scale = np.sqrt(n_antenna), theta = theta)
        else:            
            self.complex_codebook = complex_codebook # n_beams x n_antenna
            cb_blockmatrix = codebook_blockmatrix(self.complex_codebook.T)
...
#%% traning loss     
# fit(learnable codebook model, dataloader_t, 
# dataloader_v, optimizor, loss function, epoch)
def fit(model, train_loader, val_loader, opt, loss_fn, EPOCHS):
    ...
    for epoch in range(EPOCHS):
        # forward propagation (specified in traning process)
        model.train()
        train_loss = 0
        train_acc = 0
        # X_batch (batch_size, narrow_beam_number)
        # Y_batch (batch_size)
        for batch_idx, (X_batch, y_batch) in enumerate(train_loader):
            ...
            # *****clear grads*****
            optimizer.zero_grad()
            # output (batch_size, n_narrow_beam) output.shape
            output = model(var_X_batch)
            loss = loss_fn(output, var_y_batch)         # crossEntropy Loss function
            loss.backward()
            # **update params
            optimizer.step()
